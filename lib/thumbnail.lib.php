...
// 게시글보기 썸네일 생성
function get_glisse_view_thumbnail($contents, $thumb_width=0)
{
    global $board, $config;
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img class="pics" src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'"  data-glisse-big="'.G5_URL.$data_path.'" rel="group1" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img class="pics" src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'" data-glisse-big="'.G5_URL.$data_path.'" alt="'.$alt.'"/>';
            }
